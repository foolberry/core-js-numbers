...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(scalarMult / lengthMult);
}
...
function toNumber(value, def) {
  // throw new Error('Not implemented');
  if (
    (value instanceof Number || Number.isNaN(Number(value)) === false) &&
...
function getCountOfOddNumbers(number) {
  // throw new Error('Not implemented');
  const intNumber = Math.abs(number);
  return intNumber % 2 === 0 ? intNumber / 2 : Math.ceil(intNumber / 2);
}
